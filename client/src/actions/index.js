...
import { FETCH_DT, FETCH_CITY, FETCH_TEMP, FETCH_FEELS, FETCH_ICON, FETCH_DESCRIPTION, FETCH_MAX, FETCH_MIN, FETCH_HUMID, FETCH_RISE, FETCH_SET } from './types';

export const fetchWeather = () => async dispatch => {
  const res = await axios.get('/api');

  try {
    var dt = timeConvert("dt", res.data.sys.sunset, res.data.dt);

    dispatch({ type: FETCH_DT, payload: dt });
